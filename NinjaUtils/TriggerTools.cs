...
namespace NinjaUtils
{
    public class TriggerTools : MonoBehaviour
    {
        ...
        public void FixedUpdate()
        {
            ...
            if (coreHasBeenSetup && !ninjaCalls.corePuased && Reptile.Utility.GetIsCurrentSceneStage() && Reptile.Utility.GetCurrentStage() != Stage.NONE)
            {
                ...
                if (DisplayTriggerZones)
                {
                    ...
                    if (!triggersInitd) { SetupTriggerMeshes(); }
            ...
            {
                if (triggersInitd != false) { triggersInitd = false; }
            }
        }
        ...
        public void SetupTriggerMeshes()
        {
            ...
            {
                bool isTriggerObject = item != null && item.layer == 19 && item.TryGetComponent(out Collider coll) && item.name.Contains("trigger");
